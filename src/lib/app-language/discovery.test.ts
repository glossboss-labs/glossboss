...
import { parsePOFile, type POEntry } from '@/lib/po';
...
describe('discovered app languages', () => {
  ...
  it('keeps every discovered catalog in sync with the English source keys', () => {
    ...
    const englishSource = localeModules[`./locales/${englishCatalog!.filename}`];
    const englishFile = parsePOFile(englishSource, englishCatalog!.filename);
    const englishKeys = new Set(englishFile.entries.map(getEntryKey));

    for (const language of DISCOVERED_APP_LANGUAGES) {
      const rawContent = localeModules[`./locales/${language.filename}`];
      const languageFile = parsePOFile(rawContent, language.filename);
      const languageKeys = new Set(languageFile.entries.map(getEntryKey));
