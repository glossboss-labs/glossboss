    ...
    if (escaped) {
      ...
      switch (char) {
        ...
        case '\\':
          ...
          break;
        default:
          result += '\\' + char; // Keep unknown escapes
      }
      escaped = false;
    } else if (char === '\\') {
      escaped = true;
 ...
 */
function parseEntryBlock(block: RawEntry): { entry: POEntry | null; issues: ParseIssue[] } {
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    // msgctxt
      ...
      if (!valid) {
        issues.push(createIssue('error', 'UNCLOSED_QUOTE', `Unclosed quote in msgctxt`, lineNum));
      }
      entry.msgctxt = value;
    }
    // msgid
      ...
      if (!valid) {
        issues.push(createIssue('error', 'UNCLOSED_QUOTE', `Unclosed quote in msgid`, lineNum));
      }
      entry.msgid = value;
    }
    // msgid_plural
      ...
      if (!valid) {
        issues.push(
          createIssue('error', 'UNCLOSED_QUOTE', `Unclosed quote in msgid_plural`, lineNum),
        );
      }
        ...
        if (!valid) {
          issues.push(
            createIssue(
              'error',
              'UNCLOSED_QUOTE',
              `Unclosed quote in msgstr[${currentPluralIndex}]`,
              lineNum,
            ),
          );
        }
    ...
    // msgstr (singular)
      ...
      if (!valid) {
        issues.push(createIssue('error', 'UNCLOSED_QUOTE', `Unclosed quote in msgstr`, lineNum));
      }
      entry.msgstr = value;
    }
    // Continuation line (starts with ")
      ...
      if (!valid) {
        issues.push(
          createIssue('error', 'UNCLOSED_QUOTE', `Unclosed quote in continuation line`, lineNum),
        );
      }
    ...
    // Unknown line
      ...
      issues.push(
        createIssue(
          'warning',
          'INVALID_SYNTAX',
          `Unrecognized line format: "${line.substring(0, 30)}..."`,
          lineNum,
        ),
      );
    }
  ...
  if (entry.msgid === undefined) {
    issues.push(createIssue('error', 'MISSING_MSGID', `Entry is missing msgid`, startLine));
  ...
  if (entry.msgstr === undefined && !entry.msgstrPlural) {
    issues.push(
      createIssue('warning', 'MISSING_MSGSTR', `Entry has msgid but no msgstr`, startLine),
    );
    entry.msgstr = '';
  }
 ...
 */
export function parsePOFile(content: string, filename: string, options: ParseOptions = {}): POFile {
...
export function parsePOFileWithDiagnostics(
  ...
  for (let i = 0; i < blocks.length; i++) {
    ...
    if (entry.flags.includes('fuzzy')) {
      ...
      entry.msgstr.trim() ||
      (entry.msgstrPlural && entry.msgstrPlural.some((s) => s?.trim()))
    ) {
  ...
  if (entries.length === 0) {
    errors.push(
      createIssue('error', 'NO_ENTRIES', 'No translation entries found (only header present)'),
    );
    return { success: false, errors, warnings, stats };
  }
