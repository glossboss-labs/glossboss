...
const PLACEHOLDER_PREFIX = 'â¦GLOSS';
const PLACEHOLDER_SUFFIX = 'â§';
...
function detectCapitalization(text: string): 'lower' | 'upper' | 'title' | 'mixed' {
  ...
  const isTitle = text[0] === text[0].toUpperCase() && 
 ...
 */
export function prepareForTranslation(
  sourceText: string,
...
export function restoreGlossaryTerms(
  ...
  placeholders.forEach((info, id) => {
    ...
    if (result.includes(placeholder)) {
      ...
      const variations = [
        placeholder,
        placeholder.replace('â¦', '[ ').replace('â§', ' ]'),
        placeholder.replace('â¦', '[').replace('â§', ']'),
        `GLOSS${id}`,
        `Gloss${id}`,
        `gloss${id}`,
...
export function createGlossaryAwareTranslator(
  ...
  return async (sourceText: string): Promise<string> => {
    ...
    const { text: preparedText, placeholders, hasGlossaryTerms } = 
    ...
    console.log('[Glossary] Placeholders:', [...placeholders.entries()].map(([id, info]) => 
      `${id}: "${info.entry.term}" â "${info.entry.translation}" (${info.capitalization})`
    ));
    ...
    console.log('[Glossary] Applied terms:', appliedTerms.map(e => `${e.term} â ${e.translation}`));
