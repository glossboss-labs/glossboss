...
import type { DeepLGlossary } from '@/lib/deepl/types';
...
export async function syncGlossaryToDeepL(
  ...
  console.log(`[DeepL Sync] ${validEntries.length} unique entries (${wpGlossary.entries.length - validEntries.length} duplicates removed)`);
  
  // Check if we already have a glossary for this locale with same entries
  const entriesHash = hashEntries(validEntries.map(e => ({ term: e.source, translation: e.target })));
