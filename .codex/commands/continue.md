...
5. Read the implementation plan in AGENTS.md for overall context. `CLAUDE.md` and `CODEX.md`
   point to the same instructions if you need the tool-specific alias.
...
2. **While working**: Follow AGENTS.md instructions strictly — run the full CI surface (`bun run lint`, `bun run format:check`, `bun run typecheck`, `bun run build`, `bun run test:coverage`) before considering any task complete. Run `bun run i18n:extract` if any translatable strings changed.
