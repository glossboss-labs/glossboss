...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    if (!apiKey) {
      return new Response(
        JSON.stringify({ message: 'No API key configured. Please add your DeepL API key in Settings.' }),
    ...
    if (action === 'translate') {
      ...
      if (!response.ok) {
        ...
        return new Response(
          JSON.stringify({ message: `DeepL API error: ${error}` }),
    ...
    if (action === 'usage') {
      ...
      if (!response.ok) {
        return new Response(
          JSON.stringify({ message: 'Failed to fetch usage' }),
          { status: response.status, headers: { ...corsHeaders, 'Content-Type': 'application/json' } }
        );
      }
    ...
    if (action === 'createGlossary') {
      ...
      if (!name || !sourceLang || !targetLang || !entries || !entries.length) {
        return new Response(
          JSON.stringify({ message: 'Missing required fields: name, sourceLang, targetLang, entries' }),
          { status: 400, headers: { ...corsHeaders, 'Content-Type': 'application/json' } }
        );
      }
      ...
      if (!entriesTsv) {
        return new Response(
          JSON.stringify({ message: 'No valid entries provided' }),
          { status: 400, headers: { ...corsHeaders, 'Content-Type': 'application/json' } }
        );
      }
      ...
      if (!response.ok) {
        const error = await response.text();
        return new Response(
          JSON.stringify({ message: `Failed to create glossary: ${error}` }),
          { status: response.status, headers: { ...corsHeaders, 'Content-Type': 'application/json' } }
        );
      }
    ...
    if (action === 'listGlossaries') {
      ...
      if (!response.ok) {
        return new Response(
          JSON.stringify({ message: 'Failed to list glossaries' }),
          { status: response.status, headers: { ...corsHeaders, 'Content-Type': 'application/json' } }
        );
      }
    ...
    if (action === 'deleteGlossary') {
      ...
      if (!glossaryId) {
        return new Response(
          JSON.stringify({ message: 'Missing required field: glossaryId' }),
          { status: 400, headers: { ...corsHeaders, 'Content-Type': 'application/json' } }
        );
      }
      ...
      if (!response.ok && response.status !== 404) {
        return new Response(
          JSON.stringify({ message: 'Failed to delete glossary' }),
          { status: response.status, headers: { ...corsHeaders, 'Content-Type': 'application/json' } }
        );
      }
    ...
    if (action === 'getGlossary') {
      ...
      if (!response.ok) {
        return new Response(
          JSON.stringify({ message: 'Glossary not found' }),
          { status: response.status, headers: { ...corsHeaders, 'Content-Type': 'application/json' } }
        );
      }
